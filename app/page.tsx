...
import { getLocalTime as getLocalTimeUtil, formatBirthdayForDB, parseBirthday, parseVCard, mapDeviceContact } from "@/lib/utils";
...
export default function Home() {
  ...
  const [importBulkRelationship, setImportBulkRelationship] = useState("Friend");
  const [importBulkCadenceDays, setImportBulkCadenceDays] = useState(30);
  ...
  // Native Mobile Device Contact Picker API for dashboard
  ...
  // Bulk set relationship for selected contacts in dashboard review
  const applyImportBulkRelationship = (val: string) => {
    setImportBulkRelationship(val);
    setImportingContacts(
      importingContacts.map((c) => (c.selected ? { ...c, relationship: val } : c))
    );
  };

  // Bulk set cadence for selected contacts in dashboard review
  const applyImportBulkCadence = (val: number) => {
    setImportBulkCadenceDays(val);
    setImportingContacts(
      importingContacts.map((c) => (c.selected ? { ...c, cadence_days: val } : c))
    );
  };

  // Checkbox toggle in dashboard review
  ...
  // Check/uncheck all in dashboard review
  ...
  // Edit contact field in dashboard review list
  ...
  // Bulk save imported contacts to Supabase from dashboard
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      setIsAddContactOpen(false); // Close the Add Contact modal on successful import review
    } catch (err: any) {
      console.error("Dashboard Bulk Import Error:", err);
      setAddError(err.message || "Failed to import contacts");
    } finally {
  ...
  const handleStartEditTouchpoint = (tp: any) => {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white">
                  ...
                  <div className="space-y-6 animate-fadeIn">
                    ...
                    <div className="space-y-3">
                      ...
                      {selectedContact.touchpoints && selectedContact.touchpoints.length > 0 ? (
                        <div className="space-y-4">
                          {selectedContact.touchpoints.slice(0, drawerVisibleCount).map((tp: any) => (
      ...
      {/* DASHBOARD IMPORT REVIEW AND CUSTOMIZATION MODAL */}
      {isImportReviewOpen && (
        <div
          className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fadeIn"
          ...
          >
            ...
            <div className="p-4 bg-[#0d1527] border-b border-gray-850/80 shrink-0 grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  value={importBulkRelationship}
                  onChange={(e) => applyImportBulkRelationship(e.target.value)}
            ...
            <div className="px-6 py-2 bg-[#090d1a] border-b border-gray-850/80 shrink-0 flex justify-between items-center text-[10px] font-semibold text-gray-400">
              <div className="flex items-center gap-2">
                <input
                  ...
                  id="selectAllImportDashboard"
                />
                <label htmlFor="selectAllImportDashboard" className="cursor-pointer select-none">
            ...
            <div className="px-6 py-4 border-t border-gray-800/80 bg-[#0d1527] shrink-0 flex justify-end gap-3">
              ...
              <button
                ...
                disabled={importBulkSaving || importingContacts.filter((c) => c.selected).length === 0}

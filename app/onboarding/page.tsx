...
export default function OnboardingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white flex flex-col justify-start sm:justify-center items-center pt-14 pb-6 sm:py-6 px-4 relative overflow-hidden select-none">
      ...
      <main className={`w-full ${step === "setup" ? "max-w-5xl" : "max-w-2xl"} bg-[#0b1120]/45 border border-gray-800/80 backdrop-blur-md rounded-3xl p-4 sm:p-10 shadow-2xl flex flex-col justify-between min-h-[500px] max-h-[92vh] transition-all duration-300`}>
        ...
        {/* STEP 2: SPACIOUS SETUP WITH GRID */}
        {step === "setup" && (
          <div className="flex-1 flex flex-col overflow-hidden animate-fadeIn justify-between">
            {/* Scrollable Split Layout Wrapper */}
            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-8 overflow-y-auto px-4 pr-3 custom-scrollbar max-h-[62vh]">
              
              {/* Left Column: Title & Form */}
              <div className="space-y-6">
                <div>
                  <h3 className="text-xl font-bold text-white mb-0.5">
                    {editingContactId ? `Edit ${name || "Contact"}` : "Onboard Contacts"}
                  </h3>
                  <p className="text-[11px] text-slate-400">
                    {editingContactId ? "Update your contact details below." : "Add the people in your circle. Form fields match the Add Contact dashboard modal exactly."}
                  </p>
                </div>

                {/* Automate Import Action Bar - Pinned at the top of the form */}
                <div className="bg-[#111827]/40 border border-slate-800/80 rounded-2xl p-4 space-y-3">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    <div>
                      <h4 className="text-xs font-semibold text-gray-300 uppercase tracking-wider">Automate Import</h4>
                      <p className="text-[10px] text-slate-400 mt-0.5">Import your contacts by uploading a .vcf file.</p>
                    </div>
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    {(name || relationship !== "Friend" || cadenceDays !== 30 || phone || email || city || country || birthdayMonth || lastTouchpointDate || notes || editingContactId) && (
                      <button
                        ...
                        title={editingContactId ? "Cancel Editing" : "Clear Form"}
                      >
                        {editingContactId ? "Cancel" : "Clear"}
                      </button>
                    )}

                    {/* Prominent Add to Circle (+) Text button */}
                    <button
                      type="submit"
              ...
              {/* Right Column: Added Contacts List */}
              <div className="border-t md:border-t-0 md:border-l border-slate-800/80 pt-6 md:pt-0 md:pl-8 space-y-6">
                ...
                {/* List items grid */}
                {contacts.length === 0 ? (
                  <div className="bg-slate-900/10 border border-dashed border-slate-800/80 rounded-2xl p-8 text-center animate-fadeIn min-h-[140px] flex flex-col items-center justify-center">
                    <p className="text-[11px] text-slate-500 max-w-[200px] leading-relaxed mx-auto">
                      Your circle is empty. Create or import your first contact to start.
                    </p>
                  </div>
        ...
        {step === "setup" && (
          <div className="mt-6 pt-5 border-t border-slate-800/80 flex justify-between items-center shrink-0">
            ...
            {/* Finish Button - Labeled Text Button */}
            <button
